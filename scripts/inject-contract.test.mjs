 ...
 * package. Approved for the 0.1.6-alpha.2 cohort: each name is a browser
 ...
 * 0.1.6-alpha.2: it is the ui-workspace row of dsh-web-app and the provider
...
test('every dsh.client.inject name is an approved 0.1.6-alpha.2 client module', () => {
  for (const name of collectInjects()) {
    assert.ok(
      APPROVED_INJECT_MODULES.includes(name),
      `inject name outside the approved 0.1.6-alpha.2 module set: ${name}`,
    )
  }
})
