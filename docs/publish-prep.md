...
- alpha 分支的聚合包没有任何外部依赖：`dsh-better-sidebar@0.19.1`（2026-09-11 发布的稳定版本）的 peer 区间 `^0.1.5-rc.1` 不覆盖本分支的 `0.1.6-alpha.2` cohort，故 `aggregate.yml` 不再声明该行、`package.json` 不再依赖它、锁定它的 `minimumReleaseAgeExclude` 条目也已删除；右侧面板改为按需安装。稳定线 `dev` 仍内置 0.19.1 并保留对应锁定条目。
...
CI 与发布挂载冒烟测试挂载目标为 `@deepseek-ai/dsh@0.1.6-alpha.2`，与家族包通过 `dsh.engines.dsh >=0.1.6-alpha.2` 声明的最低宿主版本一致。
